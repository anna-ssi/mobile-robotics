...
from geometry_msgs.msg import Quaternion, Twist, Pose, Point, Vector3, TransformStamped, Transform, Pose2D

from duckietown.dtros import DTROS, NodeType
from duckietown_msgs.msg import WheelEncoderStamped, AprilTagDetection
...
class DeadReckoningNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        # Setup subscribers
        self.sub_apriltag_detection = rospy.Subscriber(f'/{self.veh}/augmented_reality_apriltag_node/detection',
                                                       AprilTagDetection, self.cb_apriltag_detection,
                                                       queue_size=1)

        self.sub_encoder_left = message_filters.Subscriber(
            f'/{self.veh}/left_wheel_encoder_node/tick', WheelEncoderStamped)

        self.sub_encoder_right = message_filters.Subscriber(
            f'/{self.veh}/right_wheel_encoder_node/tick', WheelEncoderStamped)
        ...
        self.timer = rospy.Timer(rospy.Duration(
            1 / self.publish_hz), self.cb_timer)
        self._print_time = 0
        ...
        self.buffer = Buffer()
        self.transform_listener = TransformListener(self.buffer)
        self.loginfo("Initialized")

    def cb_apriltag_detection(self, detection):
        # detection.transform is the transform from camera to apriltag
        tag_id = detection.tag_id
        cam_tag_translation, cam_tag_rotation = detection.transform.translation, detection.transform.rotation

        try:
            # sending the information of the predicted april tag
            self._tf_broadcaster.sendTransform(
                translation=[cam_tag_translation.x,
                             cam_tag_translation.y, cam_tag_translation.z],
                rotation=[cam_tag_rotation.x, cam_tag_rotation.y,
                          cam_tag_rotation.z, cam_tag_rotation.w],
                time=rospy.Time.now(),
                child="apriltag_{}".format(tag_id),
                parent='odometry')

            # find the inverse in odometry
            # predicted robot location with respect to the predicted apriltag
            inverse_transform = self.buffer.lookup_transform(
                "apriltag_{}".format(tag_id),
                "odometry",
                rospy.Time(0)
            )
            cam_tag_transform_inverse, cam_tag_rotation_inverse = inverse_transform.transform.translation, inverse_transform.transform.rotation

            # sending my inverse tranformation data
            # my robot's location with respect to my fixed apriltag
            self._tf_broadcaster.sendTransform(
                translation=[cam_tag_transform_inverse.x,
                             cam_tag_transform_inverse.y, cam_tag_transform_inverse.z],
                rotation=[cam_tag_rotation_inverse.x, cam_tag_rotation_inverse.y,
                          cam_tag_rotation_inverse.z, cam_tag_rotation_inverse.w],
                time=rospy.Time.now(),
                child='robot_intermidiate',
                parent="fix_{}".format(tag_id))

            # getting the robot position with respect to world
            transform_tag_cam = self.buffer.lookup_transform(
                "world",
                "robot_intermidiate",
                rospy.Time(0)
            )

            # update the x, y, theta accordingly
            rob_translation, rob_rotation = transform_tag_cam.transform.translation, transform_tag_cam.transform.rotation
            self.x, self.y, self.z = rob_translation.x, rob_translation.y, rob_translation.z
            _, _, self.yaw = tr.euler_from_quaternion([rob_rotation.x, rob_rotation.y,
                                                       rob_rotation.z, rob_rotation.w])
    ...
    def cb_ts_encoders(self, left_encoder, right_encoder):
        ...
        if dt < 1e-6:
            self.logwarn(
                "Time since last encoder message (%f) is too small. Ignoring" % dt)
            return
    ...
    def publish_odometry(self):
        ...
        self._tf_broadcaster.sendTransform(
            ...
            rotation=self.q,
            time=rospy.Time.now(),
            child=self.target_frame,
            parent=self.origin_frame)
